...
import { getSessionAsync } from "../sessionStore.js";
...
export async function buildContext({ portal, body, domain: domainHint = null }) {
  ...
  try {
    await hydrateSessionLeadFromEvent(body);
  } catch (e) {
    logger.warn({ ctx: CTX, e: String(e) }, "hydrateSessionLeadFromEvent failed (ignored)");
  }

  const portalCfg = (domain ? await getPortalAsync(domain) : null) || portal || null;
  const baseUrl = portalCfg?.baseUrl || null;
  const accessToken = portalCfg?.accessToken || null;
  const hasToken = !!accessToken;

  const session = domain && dialogId ? (await getSessionAsync(domain, dialogId)) || null : null;
  ...
  const isSimpleOem = isSystemLike ? false : isSimpleOemQuery(msg?.text || "", detectedOems);
  ...
  if (domain && leadId && baseUrl && accessToken) {
    try {
      ...
      if (oemFieldCode && leadRaw && Object.prototype.hasOwnProperty.call(leadRaw, oemFieldCode)) {
  ...
  return {
    ...
    message: {
      ...
      isSystemLike,
      chatEntityType:
        body?.data?.PARAMS?.CHAT_ENTITY_TYPE || body?.data?.params?.CHAT_ENTITY_TYPE || null,
      userFlags: {
