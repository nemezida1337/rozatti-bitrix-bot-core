...
const QUESTIONISH_REGEX =
  /(\?|подскажите|можно( ли)?|как|где|когда|сколько|есть ли|есть информация|есть новости|скиньте|пришлите|уточните|сообщите)/i;

const HOWTO_TOPIC_HINT_REGEX =
  /(заказ|оформ|оплат|достав|получ|подобр|возврат|гарант|статус|срок|сроки|цен|стоимост|сумм|денег|расч[её]т|перевод|карт|связ|созвон|телефон|адрес|самовывоз|реквизит|фото|видео|накладн|отправ|трек|идти|прид[её]т|график|время\s*работы|до\s*скольки|часы\s*работы)/i;
