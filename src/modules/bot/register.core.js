...
// (LLM-контур реализован в handler_llm_manager.js и вешается отдельно в index.js)
export async function handleOnImBotMessageAdd({ body, portal, domain }) {
  ...
  // Выбор позиций из уже найденного списка ABCP (цифры 1 3 / 2x2 и т.п.)
  const __sel = await tryHandleSelectionMessage({ api, dialogId, text });
  if (__sel) return;
