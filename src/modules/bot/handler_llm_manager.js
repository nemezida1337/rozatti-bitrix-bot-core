...
// LLM-бот + ABCP + лиды + сбор контакта.
...
export async function handleOnImBotMessageAdd({ body, portal, domain }) {
  ...
  // 2) Если мы уже отправили карточку и ждём подтверждения
  if (session.contactPromptSent && !session.contactConfirmed) {
    if (confirm) {
      ...
      try {
        ...
        const leadId = await leadsApi.ensureLeadForDialog(session, dialogMeta);
        session.leadId = leadId;
        session.leadCreated = true;
        console.log("[llm-bot] Lead ensured after contact confirm:", leadId);

        if (Array.isArray(session.selectedItems) && session.selectedItems.length) {
          try {
            await leadsApi.setProductRowsFromSelection(
              session.leadId,
              session.selectedItems
        ...
        console.error("[llm-bot] Failed to ensure lead after contact confirm:", e);
      }

      const thanks = session.name
        ? `Спасибо, ${session.name}. Я уже вижу ваш номер телефона. Уточню информацию по запчастям и свяжусь с вами.`
        : "Спасибо. Я уже вижу ваш номер телефона. Уточню информацию по запчастям и свяжусь с вами.";

      await sendMessage(rest, dialogId, thanks);
      return;
    }

    await sendContactCard({ session, rest, dialogId });
    return;
  }

  // 2.1) Если телефон уже появился, но карточку ещё не показывали
  if (!session.contactPromptSent && session.phone) {
    session.contactPromptSent = true;
    await sendContactCard({ session, rest, dialogId });
    return;
  }

  // 3) ABCP-поиск по OEM — только как источник данных для LLM
  let abcpSearch = null;
  try {
  ...
  // 4) Вызов LLM-внутреннего движка (с историей + ABCP-данными)
  const {
    replyText,
    stage,
    needOperator,
  ...
  // 5) Обновление лида, если он уже есть
  if (session.leadId) {
    try {
